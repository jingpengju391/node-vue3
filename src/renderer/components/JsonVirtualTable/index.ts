...
import { JsonVirtualContainer, JsonVirtualProps, ClassNameDelimiter, StyleType, ClassNameType, PositionKey, PropertyOrderKeyType, FunFix, ClassNameVarType } from './type'
...
export default defineComponent({
	...
	setup(props) {
		 ...
		 * row drop sort
		 */
		// @ts-ignore use rowDropSort
		let rowDropSort: Sortable | null = null // eslint-disable-line
		/**
		 * column drop sort
		 */
		// @ts-ignore use columnDropSort
		let columnDropSort: Sortable | null = null // eslint-disable-line
		...
		function _dropRow() {
			...
			const option = (rowOption ? Object.assign({}, dragRowOption, rowOption) : dragRowOption) as Sortable.SortableOptions
			rowDropSort = Sortable.create(_el, option)
		}
		...
		function _dropCol() {
			...
			const option = (colOption ? Object.assign({}, dragColOption, colOption) : dragRowOption) as Sortable.SortableOptions
			columnDropSort = Sortable.create(_el, option)
		}
	...
	render() {
		...
		className.push(
			...(<PositionKey[]>Object.keys(properties.hideWrapBorder!)).filter((key) => properties.hideWrapBorder![key]).map((key) => `${ClassNameType.hideWrapBorder}${ClassNameDelimiter.rod}${key}`)
		)
...
function __assignDefaultEvent<T, U>(defaultEvent: T, events: U): T & U {
	...
	return Object.keys(tempEvents).reduce(
		(newObj, key) => {
			const regex = new RegExp(`/^${FunFix.prefix}/`, 'i')
			const newKey = !regex.test(key) ? `${FunFix.prefix}${key.charAt(0).toUpperCase()}${key.slice(1)}` : key
